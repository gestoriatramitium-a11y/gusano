...
| Rarezas y objetos configurables            | Catálogo derivado de `src/core/balance.ts` y aplicado por `src/core/game.ts` | Pruebas de puntos, XP, pesos, escala, efectos y semilla         |
...
| Build estático y carga diferida            | Vite, `GameCanvas.tsx`                                                       | `npm run build`; chunks inicial/diferidos en `docs/BALANCE.md`  |
